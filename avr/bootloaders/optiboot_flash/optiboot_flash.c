...
unsigned const int __attribute__((section(".version"))) optiboot_version =
...
/**
 * @brief OpenBLT related variable
 *
 */
#ifdef OPENBLT_ENABLE

/**xcp protocol related*/
// max rx tx data len
#define BOOT_COM_RX_MAX_DATA 136
...
do_spm(uint16_t address, uint8_t command, uint16_t data);
...
// This is necessary on targets that where the CLKPR has been set in user
// application
...
// Fix ATmega128 avr-libc bug
      ...
      // Jump to 'save' or RST vector
          ...
          // it uses fact that optiboot_version constant is 2 bytes before end
          // of flash
...
/* get character from UART */
    ...
    /* only continue if connected */
      ...
      case XCP_CMD_GET_STATUS:
        ...
        xcpInfo.ctoData[2] = 0; // no protection
...
#ifdef RAMPZ
      // LOAD_EXTENDED_ADDRESS is needed in STK_UNIVERSAL for addressing more
      // than 128kB
      if (AVR_OP_LOAD_EXT_ADDR == getch()) {
        // get address
        getch(); // get '0'
        RAMPZ = (RAMPZ & 0x01) |
                ((getch() << 1) & 0xff); // get address and put it in RAMPZ
        getNch(1);                       // get last '0'
        // response
        putch(0x00);
      } else {
    ...
    /* Write memory, length is big endian and is in bytes */
      ...
      if (address.word == rstVect0) {
        ...
        vect.word =
            (vect.word - save_vect_num); // substract 'save' interrupt position
        // Move RESET jmp target to 'save' vector
        buff[saveVect0] = vect & 0xff;
        buff[saveVect1] = (vect >> 8) | 0xc0; //
        // Add rjump to bootloader at RESET vector
        vect.word =
            ((uint16_t)main); // (main) is always <= 0x0FFF; no masking needed.
...
void putch(char ch) {
  ...
  __asm__ __volatile__(
      ...
      "   brne 1b\n"
      :
      : [bitcnt] "d"(10), [ch] "r"(ch), [uartPort] "I"(_SFR_IO_ADDR(UART_PORT)),
...
#ifdef SOFT_UART
  watchdogReset();
  __asm__ __volatile__("1: sbic  %[uartPin],%[uartBit]\n" // Wait for start edge
                       "   rjmp  1b\n"
                       ...
                       "3:\n"
                       : [ch] "=r"(ch)
                       : [bitCnt] "d"(9), [uartPin] "I"(_SFR_IO_ADDR(UART_PIN)),
...
void uartDelay() {
  __asm__ __volatile__("ldi r25,%[count]\n"
                       "1:dec r25\n"
                       "brne 1b\n"
...
static inline void read_mem(uint8_t memtype, addr16_t address,
                            pagelen_t length) {
  uint8_t ch;
...
static void do_spm(uint16_t address, uint8_t command, uint16_t data) {
  ...
  asm volatile("    movw  r0, %3\n"
               "   sts %0, %1\n"
               "   spm\n"
               "   clr  r1\n"
               :
               : "i"(_SFR_MEM_ADDR(__SPM_REG)), "r"((uint8_t)command),
                 "z"((uint16_t)address), "r"((uint16_t)data)
               : "r0");
#else
  asm volatile("    movw  r0, %3\n"
               "   out %0, %1\n"
               ...
               "   clr  r1\n"
               :
               : "i"(_SFR_IO_ADDR(__SPM_REG)), "r"((uint8_t)command),
                 "z"((uint16_t)address), "r"((uint16_t)data)
               : "r0");
#endif
  ...
  boot_spm_busy_wait();
#if defined(RWWSRE)
  // this 'if' condition should be: (command == __BOOT_PAGE_WRITE || command ==
  // __BOOT_PAGE_ERASE)... but it's tweaked a little assuming that in every
  // command we are interested in here, there must be also SELFPRGEN set. If we
  // skip checking this bit, we save here 4B
...
void do_spm_rampz(uint32_t address, uint8_t command, uint16_t data) {
  ...
  do_spm((address & 0xffff), command,
         data); // do_spm accepts only lower 16 bits of address
#else
  do_spm(address, command,
         data); // 16 bit address - no problems to pass directly
#endif
}

/*
 * Function copy_flash_pages uses do_spm() function to copy flash pages.
 * It is intended to be called by the application over the 'vector table' in
 * pre_main(). It uses 32bit addresses for use on devices with more then 64 kB
 * flash memory. The destination and source address must be page aligned.
 * Additionally parameter reset_mcu activates an (almost) immediate watchdog
 * reset of the MCU after pages are copied.
 *
 * It was created to copy a new version of the aplication stored in the upper
 * half of the flash memory to the beginnig of the flash and then reset the MCU
 * to run the new version. It is used by ArduinoOTA libray in InternalStorageAVR
 * over utility/optiboot.h.
 */
void copy_flash_pages(uint32_t dest_page_addr, uint32_t src_page_addr,
                      uint16_t page_count, uint8_t reset_mcu) {
  int i, j;
  for (i = 0; i < page_count; i++) { // do standard spm steps for every page
    ...
    for (j = 0; j < SPM_PAGESIZE; j += 2) { // fill the bytes for the page
      ...
      do_spm_rampz(dest_page_addr + j, __BOOT_PAGE_FILL,
                   pgm_read_word_far(src_page_addr + j));
#else
      do_spm(dest_page_addr + j, __BOOT_PAGE_FILL,
             pgm_read_word(src_page_addr + j));
#endif
    }
  ...
  if (reset_mcu) {
    ...
    while (1)
      ; // to prevent return to application in the 15MS to reset
  }
}
#endif
